...
struct MenuItemList: View {
    ...
    init(items: [MenuItem] = MenuItem.allCases, selection: Binding<MenuItem?>, isOpened: Binding<Bool>, user: User = .stub, onUserSelected: (() -> Void)? = nil) {
        self.items = items
        self.user = user
...
struct MenuItemList_Previews: PreviewProvider {
    ...
    private struct Preview: View {
        ...
        var body: some View {
            MenuItemList(selection: $selection, isOpened: $isOpened) {
